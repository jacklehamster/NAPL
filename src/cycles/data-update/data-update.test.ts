...
describe("commitUpdates", () => {
  ...
  it("should sort updates by confirmed timestamp and apply them", () => {
    ...
    expect(
      Object.fromEntries(
        [...updatedPaths.entries()].map(([key, update]) => [key, update.value]),
      ),
    ).toEqual({
      a: {
        b: 2,
  ...
  it("should apply update when value is object", () => {
    ...
    commitUpdates(
      {
        root: obj,
        incomingUpdates: [{ path: "abc", value: { a: 1 }, confirmed: 1 }],
        properties: {},
        outgoingUpdates: [],
      },
      updatedPaths,
  ...
  it("should delete element when value is undefined", () => {
    ...
    commitUpdates(
      {
        root: obj,
        incomingUpdates: [{ path: "abc", value: undefined, confirmed: 1 }],
        properties: {},
        outgoingUpdates: [],
      },
      updatedPaths,
